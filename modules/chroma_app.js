...
class Spaces {
    ...
    genSpaces(ctx) {
        ...
        for (let i = 0; i < this.N; i++) {
            GAMEINFO.selectedArr[i] ? (isChroma = true) : (isChroma = false);
            ctx.save();
            ctx.beginPath();
            ctx.fillStyle = isChroma === true ? GAMEINFO.selectedArr[i] : "#cacffc";
            ctx.rect(
                this.x,
...
export default class ChromaGame {
    ...
    onDown(e) {
        ...
        if (this.isDown) {
            ...
            if (!GAMEINFO.getAnswerArr.includes(ColorD2X(pickColor)) ||
                GAMEINFO.getGivenArr.includes(ColorD2X(pickColor))
            ) {
                pickColor = -1;
            }
    ...
    gradeChromaGame() {
        ...
        if (
            corrAns ===
            GAMEINFO.getAnswerArr.length -
            GAMEINFO.getGivenArr.filter((el) => el != false).length
        ) {
            return true;
        } else {
            return false;
