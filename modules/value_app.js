 ...
 * @param {array} optionArr value 버튼 배열 - 랜덤으로 설정되는 기준색에 의해 결정
...
class Spaces {
  ...
  dropSelectedColor(posX, posY, selectedColor, basisLoc) {
    ...
    let checkSpace = false; //사용자가 선택한 명도를 채워넣을 공간이 맞는지 확인하는 변수
    let loc = 0; //사용자가 선택한 명도를 떨어뜨린 칸의 위치 인덱스를 담는 변수
    ...
    if (!isNoDropAreaCheck && checkSpace) {
      ...
      if (!GAMEINFO.selectedArr.includes(selectedColorCode)) {
        //1) 명도 btns에서 명도를 가져와 space에 넣는 경우
        ...
        if (!GAMEINFO.selectedArr[loc]) {
          ...
          //1-2) drop할 칸에 이미 명도가 채워져 있는 경우 - switch
          ...
          GAMEINFO.optionArr.push(temp); //명도 btns에 임시 변수에 담긴 변수 넣음
          GAMEINFO.optionArr.splice(remove, 1); //optionArr에서 user-value 제거
        }
      } else {
...
export default class ValueGame {
   ...
   */
  constructor(query, N) {
    this.currentStage = N <= 10 ? 1 : 2; //명도 수를 바탕으로 현재 스테이지 결정
  ...
  onDown(e) {
    ...
    this.isDown = true; //사용자의 클릭 상태를 저장하는 변수
