...
class Wheel {
  ...
  dropSelectedColor(posX, posY, selectedColor) {
    ...
    for (let i = 0; i < this.N; i++) {
      ...
      if (dist(x, y, relativeX, relativeY) < r) {
        //this area has index of i
        ...
        if (onWheelArr[i] == false) {
          // if the area not already filled
          if (colorFromBtn != -1) {
            // if the color is from the button
            onBtnArr.splice(colorFromBtn, 1);
          } else {
            // if the color is from the other area of the wheel
            delete onWheelArr[colorFromWheel];
            onWheelArr[colorFromWheel] = false;
          }
        } else {
          // if the area already filled
