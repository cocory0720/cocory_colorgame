...
const GAMEINFO = {
  ...
  set currentGame(game) {
    ...
    if (gameList.indexOf(game) == -1) {
      throw "bad request in setting current game";
    }
  ...
  get answerArr() {
    if (this.currentGame === undefined || this.currentStage === undefined) {
      console.log("initialize current game n stage first");
      return;
    }
  ...
  initColorWheelAngles(n) {
    ...
    this._commonCircle =
      PI2 / (10 + (n * 3) / 4 + (howBigisGiven - 1) * numOfGiven);
  ...
  function (data) {
    ...
    for (let row = 0; row < allRows.length; row++) {
      ...
      if (allRows[row].slice(0, 2) != "//" && allRows[row] ? 1 : 0) {
        ...
        if (
          GAMEINFO[currentRow[0]][currentRow[1]][currentRow[2]] === undefined
        ) {
          /**
          ...
          if (currentRow[2] == "timeLimit") {
            GAMEINFO[currentRow[0]][currentRow[1]][currentRow[2]] = parseInt(
              currentRow[3].slice(0, -3)
            );
          } else {
